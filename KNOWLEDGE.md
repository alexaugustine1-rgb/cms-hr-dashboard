...
**Joiner gap closed.** Cycle 11-17 Aug joiners went 2 -> 19, Aug MTD 57. The 12-17 Aug hole was purely upload lag, as diagnosed. `loadEMJoinerCounts()` (~line 10062) filters `emp_status='NewJoinee'` AND `doj` in period.

**BUG 1 (fixed) — `sync_headcount_from_employee_master()` counted every row ever loaded.** The `employee_master` upsert never deletes, so 224 rows from earlier uploads survived alongside the 2,628 current ones (table total 2,852). The RPC ran `FROM employee_master GROUP BY customer, region` with **no batch filter** and wrote that straight into `customer_accounts.headcount`. 64 accounts were overstated by 192 heads in total. Worst: Exide Life South 19 vs 9, Reliance Communications West 15 vs 5, SBI General West 132 vs 120, Torrent Power West 56 vs 44, HDB West 98 vs 87.
- **Why it mattered:** Account Health deployment gap = `(contracted_hc - (active_hc - absent)) / contracted_hc`. Inflated `active_hc` shrinks the gap, so accounts read **healthier than they are**. Wrong direction, and compounding on every upload since commit 0bf2e6e.
- **Migration:** `fix_headcount_sync_current_batch_only_20260817`. Scopes the CTE to `uploaded_at >= max(uploaded_at) - interval '6 hours'` (tolerates a slow upload or one crossing midnight, excludes last week's batch). Adds a second pass that zeroes accounts with `updated_by='em_sync'` that have no rows in the current batch, so accounts emptied of staff stop carrying a stale number. Adds a guard: if the batch is under 500 rows it logs a warning and does nothing, so a partial parse can never wipe every headcount.
- **Signature deliberately kept `RETURNS integer`.** Changing it to jsonb needs a DROP, and the project is shared with OPS360 — do not change this signature without checking their callers.
- **Verified:** first run changed 66 rows (64 updated + 2 zeroed), second run returned 0 (idempotent). Only 2 of the 17 stranded accounts were zeroed because the rest are not `em_sync`-owned, which is the guard working as designed.

**BUG 2 (fixed) — `getEmpType()` silently dropped every Contract employee.** Log read `Contract: 0` against 89 in June. The test was `t==='CONTRACT'` after upper-casing, so this is a renamed ZingHR value, not a casing change. On no match the function returned `null` and the row was skipped **with no warning at all**.
- Widened to `t.indexOf('CONTRACT')!==-1`, placed **after** the FTC test so `FIXED TERM CONTRACT` still resolves to FTC. Catches CONTRACTUAL, CONTRACT STAFF, SUB-CONTRACTOR.
- Added `_unknownEmpTypes` accumulator + a red log line listing unrecognised values (excluding FTE/GENERAL/PERMANENT). **The next upload will name the actual string.** Feed it back here.
- Headcount was never affected: `activeRows` filters on the `Employee Status` column, which is separate from `EmpType`.

**BUG 3 (fixed) — `submitRMGNewPosition()` hardcoded `req_status:'Open'`, defeating an existing guard.** The manual-insert payload (~line 10967) wrote `req_status:'Open', req_age_days:0`. `req_status` is owned by `sync_positions_from_req_tracker()` and must not be hand-edited; the manual field is `status`. `_rmgEffStatus()` (~line 11014) already returns `'Not in ZingHR'` when `req_status` is null — precisely the right rendering for a req RMG has raised but ZingHR has not yet exported. The hardcode bypassed it, so such reqs counted as Open immediately and RMG Workspace disagreed with TA Pipeline.
- **Fix:** `req_status: null, req_age_days: null`. Unconfirmed reqs now render 'Not in ZingHR' (purple left border), stay out of the Open count, and are promoted to Open by Step A of the sync RPC on the next TAT upload.
- **Note the function name:** the earlier debt entry called this `addRMGPosition()`. The real function is **`submitRMGNewPosition()`** at ~line 10934. There is a second, similar-looking insert at ~line 8890 inside `linkResignationReq()` which also sets `req_status:'Open'` — it was **not** changed here and is worth a separate look, since the same reasoning may apply.
- **Stale rows cleaned:** ids 1033 and 1034 (ZingHR reqs 6922, 6917, Torrent Power, raised by Pruthvi SS on 17 Aug) were inserted before the fix and still carried `req_status='Open'`. Nulled via SQL with `updated_by='parity_fix_20260817'`. Max `req_id` in `req_tracker` was 6889, so both reqs are genuine and simply ahead of the export.
- **Parity confirmed:** `req_tracker` Open = **125**, `account_positions_log` `req_status='Open'` = **125**.

**index.html:** 16,964 -> 16,982 lines. Inherited getEmpType work +13 (14 ins / 1 del); RMG fix +5 (7 ins / 2 del). Three hunks total, no rewrite. `node --check` clean via the 2-inline-script extraction. All four overwrite-prevention greps positive (saveODAction 3, loadWFHODActions 8, _wfhOdActions 7, multiBadge 2).

**`vercel.json`:** the line-endings-only diff noted below was already clean in the working tree by the time this session ran — `git status` did not list it, so no `checkout` was needed. The no-cache headers are untouched.

**Also observed, not fixed:** `vercel.json` shows a whole-file diff that is line-endings only, content identical — `git checkout -- vercel.json` before committing to keep the diff clean. `ate_tracker` holds 81 Active against 80 written today; `contract_workforce` holds 466 rows against 416 written (the extra 50 are the known stale ATE rows, untouched since 14 Apr). 11 active employees carry no region, so regional cuts sum to 2,618 against an Active HC of 2,629. Active HC 2,749 (09 Jun) -> 2,629 (17 Aug); the old 40-row gap between `activeRows` and WI HC has collapsed to 1, worth confirming that is real and not a changed filter.
